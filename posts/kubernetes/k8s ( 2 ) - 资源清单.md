...
类似通过`Dockerfile`来表示容器的创建过程，`k8s`里的各种资源也可以通过文本的方式来创建，通常是通过`Yaml`方式来定义，也支持`Json`。本文档主要介绍资源清单的基本使用，至于具体资源清单的使用在后续使用到在进行介绍。
...
这两个步骤完成后，就可以访问`http://localhost:38002`了。上面就是资源清单的基本写法。

# 三、Yaml基础字段

配置清单的写法如上面示例，不同的资源对象大同小异，具体资源的写法在使用到会在进行介绍，先来看看通用规则。通过`kubectl explain pod`的方式可以查看对应资源的说明文档，要看子节点可以用：`kubectl explain pod.spec`，所有Yaml中的定义的字段都可以通过该方法查看文档。
